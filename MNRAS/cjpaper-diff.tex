...
\begin{equation}
\label{eq:JJ}
\JJ = %DIFDELCMD < \Matrix{cc}{\JJ_k & \JJ^*_k \\ \bar{\JJ^*_k} & \bar{\JJ_k} }%%%
\Matrix{cc}{\JJ_k & \JJ_{k^*} \\ \bar{\JJ}_{k^*} & \bar{\JJ}_{k} },
\end{equation}
...
\begin{equation}
\label{eq:Jk}
J_k = \overbrace{ \Matrix{c}{ m_{pq}\bar{g}_q\delta^{j}_p } } ^{j=1\dots \Na} %DIFDELCMD < \big \}{\scriptstyle [pq]=1\dots \Nbl~(p<q)}
%DIFDELCMD < %%%
...
\begin{equation}
\label{eq:Jbark}
J_{k^*} = \overbrace{ \Matrix{c}{ g_p m_{pq} \delta^{j}_q } } ^{j=1\dots \Na} %DIFDELCMD < \big \}{\scriptstyle [pq]=1\dots \Nbl~(p<q)}
%DIFDELCMD < %%%
\bm{ \}} {\scriptstyle [pq]=1\dots \Nbl~(p<q)}
\end{equation}
...
& \bigg ] &
%DIFDELCMD < \Stack{ \} %%%
\Stack{ \bm{\}} {\scriptstyle [pq]=1\dots \Nbl}~(p<q) \\ %DIFDELCMD < \} %%%
\bm{\}} {\scriptstyle [pq]=1\dots \Nbl}~(p<q) }
\end{array}
\end{equation}
...
\DIFaddbeginFL \DIFaddFL{This is a set of $2\times2$ matrix equations, rather than the vector equations employed in the complex NNLS formalism 
...
and then derive the complex Jacobian in the usual manner (Eq.~\ref{eq:JJ}). We may, however, obtain a more elegant 
derivation by using a simple operator calculus, where the ``atomic'' elements are $2\times2$ matrices rather than scalars. 
We can then define the Jacobian in a more transparent way, as a matrix of linear operators on such atomic elements.}
\DIFaddend
...
way. For a $2\times2$ matrix:}\DIFaddend
\begin{equation}
...
$\COMPLEX^4$. Note that the ``vec'' operator is linear. Therefore, any linear operator on 
$\mat{X} \in \COMPLEX^{2\times2}$, which we'll write as $\mathcal{B}[\mat{X}]$,  must then be equivalent to 
multiplication of $\VEC{\mat{X}} \in \COMPLEX^4$ by some $4\times 4$ complex matrix $\mat B$, and vice versa:
}\DIFaddend
\begin{equation}
\mathbf{x} = \VEC{\mat{X}},~~\mat{B}\mathbf{x} = \VEC{\mathcal{B}[\mat{X}]}
\end{equation}
\DIFaddbeginFL \DIFaddFL{
To put this formally, we can say that the  $\mathrm{vec}$ operator induces an isomorphism  $\mathcal{W}$ 
between the space of linear operators on $\COMPLEX^4$ -- that is, $\COMPLEX^{4\times4}$, and the space of linear operators on $\COMPLEX^{2\times2}$:
}\DIFaddend
\DIFaddend\begin{eqnarray}
&& \COMPLEX^{4\times4}=\mathrm{Lin}(\COMPLEX^4,\COMPLEX^4) \overset{\mathcal{W}}{\rightarrow} \mathrm{Lin}(\COMPLEX^{2\times2},\COMPLEX^{2\times2}) \nonumber\\
...
~~ 
\Stack{ 
\bm{\}} \scriptstyle [pq]=1\dots \Nbl~(p<q) %DIFDELCMD < \\ 
%DIFDELCMD < \} %%%
\\[1ex] 
\bm{\}} \scriptstyle [pq]=1\dots \Nbl~(p<q) 
}
