...
\setlength\dashlinedash{0.2pt} %DIF > 
\setlength\dashlinegap{1.5pt} %DIF > 
\setlength\arrayrulewidth{.5pt} %DIF > 
 %DIF > 
%DIF -------
...
\begin{equation}
\label{eq:Jkrk}
\JJ_k=\frac{\partial \vv}{\partial \zz} (\Zz_k), ~\JJ\DIFdelbegin \DIFdel{_k^*}\DIFdelend \DIFaddbegin \DIFadd{_{k^*}}\DIFaddend =\frac{\partial \vv}{\partial \bar{\zz}}(\Zz_k),~
\Rr_k=\Matrix{c}{\rr(\Zz_k)\\\bar{\rr}(\Zz_k)}
\end{equation}
We'll call the $m\times n$ matrices $\JJ_k$ and \DIFdelbegin \DIFdel{$\JJ_k^*$ }\DIFdelend \DIFaddbegin \DIFadd{$\JJ_{k^*}$ }\DIFaddend the \emph{partial} and \emph{partial conjugate 
...
\begin{equation}
\label{eq:JJ}
\JJ = \DIFdelbegin %DIFDELCMD < \Matrix{cc}{\JJ_k & \JJ^*_k \\ \bar{\JJ^*_k} & \bar{\JJ_k} }%%%
\DIFdelend \DIFaddbegin \Matrix{cc}{\JJ_k & \JJ_{k^*} \\ \bar{\JJ}_{k^*} & \bar{\JJ}_{k} }\DIFaddend ,
\end{equation}
\DIFdelbegin %DIFDELCMD < {\bf %%%
\DIFdelend with the bottom two blocks being element-by-element conjugated versions of the top two. Note \DIFdelbegin \DIFdel{that }\DIFdelend \DIFaddbegin \DIFadd{the use of $\bar{\JJ}$ to
indicate element-by-element conjugation -- not to be confused with the Hermitian conjugate which we'll invoke later. 
}\DIFaddend $\JJ$ is a $2m \times 2n$ matrix. \DIFdelbegin %DIFDELCMD < } %%%
\DIFdelend The GN update step is defined as
\begin{equation}
\label{eq:GN}
...
\DIFdelbegin \DIFdel{ensures that the results are consistent, i.e.}\DIFdelend \DIFaddbegin \DIFadd{is symmetric (since the function being minimized -- the Frobenius norm -- is real and symmetric w.r.t. $\zz$ and $\bar{\zz}$),
which ensures }\DIFaddend that $\overline{\delta\zz} = \delta \bar{\zz}$. In practice this  redundancy usually means that only half the calculations need to be performed.
...
\section{\DIFdelbegin \DIFdel{Unpolarized }\DIFdelend \DIFaddbegin \DIFadd{Scalar (unpolarized) }\DIFaddend calibration}
\label{sec:unpol}

In this section we will apply the formalism above to the \DIFdelbegin \DIFdel{simpler case}\DIFdelend \DIFaddbegin \DIFadd{scalar case, i.e. that }\DIFaddend of unpolarized calibration. 
...
$pq$ ($1\leq p<q\leq \Na$) 
measures the visibility\DIFdelbegin %DIFDELCMD < {\bf%%%
...
\begin{equation}
\label{eq:Jk}
J_k = \overbrace{ \Matrix{c}{ m_{pq}\bar{g}_q\delta^{j}_p } } ^{j=1\dots \Na} \DIFdelbegin %DIFDELCMD < \big \}{\scriptstyle [pq]=1\dots \Nbl~(p<q)}
%DIFDELCMD < %%%
...
\begin{equation}
\label{eq:Jbark}
J_{k^*} = \overbrace{ \Matrix{c}{ g_p m_{pq} \delta^{j}_q } } ^{j=1\dots \Na} \DIFdelbegin %DIFDELCMD < \big \}{\scriptstyle [pq]=1\dots \Nbl~(p<q)}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bm{ \}} {\scriptstyle [\DIFadd{pq}]\DIFadd{=1}\dots \Nbl\DIFadd{~(p<q)}}
\DIFaddend \end{equation}
...
& \bigg ] &
\DIFdelbegin %DIFDELCMD < \Stack{ \} %%%
\DIFdelend \DIFaddbegin \Stack{ \bm{\}} \DIFaddend {\scriptstyle [pq]=1\dots \Nbl}~(p<q) \\ \DIFdelbegin %DIFDELCMD < \} %%%
...
\Rr = \big [ r_{pq} \big ] ~~{\DIFdelbegin %DIFDELCMD < \} %%%
...
%DIFDELCMD < \Matrix{c@{}c@{}c}{
%DIFDELCMD <  #1 & \bigg |~ & #2 \\[10pt]
%DIFDELCMD <  \hline \\[-8pt]
%DIFDELCMD <  #3 & \bigg |~ & #4 }
%DIFDELCMD < }
%DIFDELCMD < %%%
...
\label{eq:JHJ:DI}
\JJ^H \JJ = 
\DIFdelbegin %DIFDELCMD < \JHJblocksFull{
%DIFDELCMD < \mathrm{diag} \sum\limits_{q\ne i} |y_{iq}^2| 
%DIFDELCMD < }{
%DIFDELCMD <   \left \{ 
%DIFDELCMD <   \begin{array}{@{}cc@{}}
%DIFDELCMD <    \bar{y}_{ij} \bar{y}_{ji}, &{\scriptstyle i\ne j} \\
%DIFDELCMD <    0, &{\scriptstyle i=j}
%DIFDELCMD <   \end{array} \right .
%DIFDELCMD < }{
%DIFDELCMD <   \left \{ 
%DIFDELCMD <   \begin{array}{@{}cc@{}}
%DIFDELCMD <    y_{ij}y_{ji},&{\scriptstyle i\ne j} \\
%DIFDELCMD <    0, &{\scriptstyle i=j}
%DIFDELCMD <   \end{array} \right . 
%DIFDELCMD < }{
%DIFDELCMD <   \mathrm{diag} \sum\limits_{q\ne i} |y^2_{iq}| 
...
%DIFDELCMD < Equation~\ref{eq:JHJ:3ant} in the Appendix provides an example of $\JJ^H\JJ$ for the 3-antenna case. 
%DIFDELCMD < 
...
%DIFDELCMD < The other component of the LM/GN equations (Eq.~\ref{eq:LM}) is the $\JJ^H\Rr$ term. This will be a column vector of length $2\Na$. We can write this as a stack of two $\Na$-vectors:
\begin{equation}%DIFAUXCMD
%DIFDELCMD < \begin{equation}
%DIFDELCMD < \label{eq:JHR:DI}
%DIFDELCMD < \JJ^H\Rr = \Matrix{c}{ 
%DIFDELCMD < \sum\limits_{pq} \bar{y}_{pq} r_{pq} \delta^{i}_p  \\
%DIFDELCMD < \sum\limits_{pq} y_{qp} r_{pq} \delta^{i}_q 
%DIFDELCMD < } = \Matrix{c}{
%DIFDELCMD < \sum\limits_{q\ne i} \bar{y}_{iq} r_{iq}   \\
%DIFDELCMD < \sum\limits_{q\ne i} y_{iq} \bar{r}_{iq}  
%DIFDELCMD < }
%DIFDELCMD < \Stack{
%DIFDELCMD < \big \} \scriptstyle i=1\dots \Na \\ 
%DIFDELCMD < \big \} \scriptstyle i=1\dots \Na
%DIFDELCMD < }
%DIFDELCMD < %%%
...
%DIFDELCMD <   \left \{ 
%DIFDELCMD <   \begin{array}{@{}cc@{}}
%DIFDELCMD <    \sum\limits_{s} y_{ijs}y_{jis},&{\scriptstyle i\ne j} \\
%DIFDELCMD <    0, &{\scriptstyle i=j}
%DIFDELCMD <   \end{array} \right . 
%DIFDELCMD < },
\end{equation}%DIFAUXCMD
%DIFDELCMD < \end{equation}
%DIFDELCMD < where the symbols $\searrow$ and $\nearrow^H$ represent a copy and a copy-transpose of the appropriate matrix 
%DIFDELCMD < block (as per the structure of Eq.~\ref{eq:JHJ:DI:ABCD}). Likewise, the $\JJ^H\Rr$ term can be written as:
%DIFDELCMD < \begin{equation}
%DIFDELCMD < \label{eq:JHR:DI:tf}
%DIFDELCMD < \JJ^H\Rr 
%DIFDELCMD < = \Matrix{c}{
%DIFDELCMD < \sum\limits_{q\ne i,s} \bar{y}_{iqs} r_{iqs}   \\
%DIFDELCMD <  \hline \\[-8pt]
%DIFDELCMD < \downarrow^H
%DIFDELCMD < %\sum\limits_{q\ne i,s} y_{iqs} \bar{r}_{iqs}  
%DIFDELCMD < }.
%DIFDELCMD < \end{equation}
%DIFDELCMD < 

%DIFDELCMD < \subsection{Weighting}
%DIFDELCMD < \label{sec:DI:W}
%DIFDELCMD < 

%DIFDELCMD < Although \citet{ComplexOpt} do not mention this explicitly, it is straightforward to incorporate weights into the 
%DIFDELCMD < complex LS problem. Equation~\ref{eq:LSmin} is reformulated as
%DIFDELCMD < \begin{equation}
%DIFDELCMD < \label{eq:LSmin:w}
%DIFDELCMD < \min_{\bmath{z}} ||\mat{W} \Rr(\bmath{z},\bmath{\bar z})||_F,
%DIFDELCMD < \end{equation}
%DIFDELCMD < where $\mat{W}$ is an $M\times M$ weights matrix (usually, the inverse of the data covariance matrix $\mat{C}$). This then propagates into the LM equations as
%DIFDELCMD < \begin{equation}
%DIFDELCMD < \label{eq:LM:W}
%DIFDELCMD < \delta\Zz = (\JJ^H \mat{W} \JJ + \lambda\II)^{-1}\JJ^H \mat{W} \Rr_k.
%DIFDELCMD < \end{equation}
%DIFDELCMD < 

%DIFDELCMD < Adding weights to Eqs.~\ref{eq:JHJ:DI:tf} and \ref{eq:JHR:DI:tf}, we arrive at the following:
\begin{equation}%DIFAUXCMD
%DIFDELCMD < \begin{equation}
%DIFDELCMD < \label{eq:JHJ:DI:tfw}
%DIFDELCMD < \JJ^H\mat{W}\JJ = 
%DIFDELCMD < \JHJblocks{
%DIFDELCMD <   \mathrm{diag} \sum\limits_{q\ne i,s} w_{iqs} |y^2_{iqs}|
%DIFDELCMD < }{
%DIFDELCMD <   \left \{ 
%DIFDELCMD <   \begin{array}{@{}c@{,~}c@{}}
%DIFDELCMD <    \sum\limits_{s} w_{ijs} y_{ijs}y_{jis}&{\scriptstyle i\ne j} \\
%DIFDELCMD <    0 &{\scriptstyle i=j}
%DIFDELCMD <   \end{array} \right . 
%DIFDELCMD < }
%DIFDELCMD < % \Stack{ 
%DIFDELCMD < % \bigg \} \scriptstyle i=1\dots \Na \\ 
%DIFDELCMD < % \bigg \} \scriptstyle i=1\dots \Na 
%DIFDELCMD < %}
%DIFDELCMD < %%%
...
\label{eq:JHR:DI:tfw}
\JJ^H\mat{W}\Rr 
= \DIFdelbegin %DIFDELCMD < \Matrix{c}{
...
%DIFDELCMD < Equation~\ref{eq:stefcal} 
%DIFDELCMD < is identical to the update step proposed by \citet{Stefcal} for the \StefCal\ algorithm, and by 
%DIFDELCMD < \citet{Mitchell-RTS} for MWA calibration. Note that these authors arrive at the result from a different 
...
%DIFDELCMD < \end{equation}
%DIFDELCMD < Here, $\DD_{pq}$ is the \emph{visibility matrix} observed by baseline $pq$, $\MM_{pq}$ is the sky \emph{coherency matrix}, $\GG_p$ is the Jones matrix associated with antenna $p$, and $\mat{N}_{pq}$ is a noise matrix. Quite importantly, the visibility and coherency matrices are Hermitian: 
...
%DIFDELCMD < In principle, this can be recast into an ordinary complex NLLS problem (Eq.~\ref{eq:LSmin}) {\bf by vectorizing each 
%DIFDELCMD < matrix equation,} and then deriving the complex Jacobian according to Eq.~\ref{eq:JJ}. We may, however, 
%DIFDELCMD < obtain a more elegant derivation by using a simple operator calculus, where the ``atomic'' elements are $2\times2$ matrices 
%DIFDELCMD < rather than scalars. We can then define the Jacobian more simply in terms of operators on such matrices. 
%DIFDELCMD < 
...
%DIFDELCMD < \subsection{Operator calculus}
%DIFDELCMD < 

%DIFDELCMD < First, let us introduce the vectorization operator ``vec'' and its inverse in the usual (stacked columns) 
%DIFDELCMD < way. For a $2\times2$ matrix:
%DIFDELCMD < \begin{equation}
%DIFDELCMD < \VEC{\mat{X}} = \Matrix{c}{x_{11}\\x_{21}\\x_{12}\\x_{22}},~~~
%DIFDELCMD < \VECINV{\Matrix{c}{x_{11}\\x_{21}\\x_{12}\\x_{22}}} = \mat{X},
%DIFDELCMD < \end{equation}
%DIFDELCMD < which sets up a trivial isomorphism between the space of $2\times2$ complex matrices $\COMPLEX^{2\times2}$ and the space 
%DIFDELCMD < $\COMPLEX^4$. Any linear operator on $\mat{X} \in \COMPLEX^{2\times2}$, which we'll write as $\mathcal{B}[\mat{X}]$, 
%DIFDELCMD < must then be equivalent to  multiplication of $\VEC{\mat{X}} \in \COMPLEX^4$ by some $4\times 4$ complex matrix $\mat B$. To put this formally, we can say that the $\mathrm{vec}$ operator induces an isomorphism  $\mathcal{W}$ 
%DIFDELCMD < between $\COMPLEX^{4\times4}$ and the space of linear 
%DIFDELCMD < operators on $\COMPLEX^{2\times2}$:
%DIFDELCMD < \begin{eqnarray}
%DIFDELCMD < && \mathcal{W}:\COMPLEX^{4\times4}\leftrightarrow\mathrm{Lin}(\COMPLEX^{2\times2},\COMPLEX^{2\times2}) \nonumber\\
%DIFDELCMD < && \mathcal{W}(\mat{B})[\mat{X}] = \VECINV{(\mat{B}\,\VEC{\mat{X}})} \nonumber \\
%DIFDELCMD < && \mathcal{W}^{-1}(\mathcal{B})\bmath{x} = \VEC{(\mathcal{B}[ \VECINV{\bmath{x}} ])} \label{eq:iso:W}
%DIFDELCMD < \end{eqnarray}
%DIFDELCMD < 

%DIFDELCMD < Of particular interest to us are two linear operators on $\COMPLEX^{2\times2}$: right multiply by some $2\times2$ complex matrix $\mat{A}$, and left multiply by $\mat{A}$:
%DIFDELCMD < \begin{equation}
...
%DIFDELCMD < \end{array}
%DIFDELCMD < \end{equation}
%DIFDELCMD < The $\mathcal{W}$ isomorphism ensures that these operators can be represented as multiplication of 4-vectors 
%DIFDELCMD < by specific kinds of $4\times4$ matrices. The exact form of these matrices is derived in Appendix~\ref{app:4x4}.
%DIFDELCMD < We may repurpose the symbols $\Rop{\mat{A}}$ and $\Lop{\mat{A}}$ to also represent the $4\times4$ matrices themselves. 
%DIFDELCMD < 

%DIFDELCMD < From the operator definitions, it is trivial to see that
%DIFDELCMD < \begin{equation}
...
%DIFDELCMD < \subsection{Derivative operators and Jacobians}
%DIFDELCMD < 

%DIFDELCMD < The vec operator also produces a natural mapping between any matrix-valued function of a 
%DIFDELCMD < matrix argument $\mat{F}(\mat{G})$, $\mat{F}:\COMPLEX^{2\times2}\to\COMPLEX^{2\times2}$ and a corresponding 
%DIFDELCMD < vector-valued function of a vector, $\bmath{f}:\COMPLEX^4\to\COMPLEX^4$:
%DIFDELCMD < \begin{equation}
%DIFDELCMD < \bmath{f}(\bmath{g}) = \VEC{\mat{F}(\VECINV{\bmath{g}})}.
%DIFDELCMD < \end{equation}
%DIFDELCMD < Consider the partial and conjugate partial Jacobians of $\bmath{f}$ with respect to $\bmath{g}$ 
%DIFDELCMD < and $\bar{\bmath{g}}$. These are $4\times4$ matrices given by Eq.~\ref{eq:Jk}:
%DIFDELCMD < \begin{equation}
%DIFDELCMD < \label{eq:Jk-fg}
%DIFDELCMD < \JJ_k = [ \partial f_i / \partial g_j ],~~~\JJ_{k^*} = [ \partial f_i / \partial \bar{g}_j ],
%DIFDELCMD < \end{equation}
%DIFDELCMD < that represent linear approximations to $\bmath{f}$, i.e. linear operators on $\COMPLEX^4$ 
%DIFDELCMD < mapping an increment in the argument $\Delta\bmath{g}$ to an increment in the function value $\Delta\bmath{f}$. 
%DIFDELCMD < The isomorphism above matches them up with equivalent linear operators on $\COMPLEX^{2\times2}$ that represent
%DIFDELCMD < linear approximations to $\bmath{F}$. We can call these operators the Wirtinger $2\times2$ matrix derivatives 
%DIFDELCMD < of $\mat{F}$ with respect to $\mat{G}$ and $\mat{G^H}$:
%DIFDELCMD < \begin{equation}
%DIFDELCMD < \label{eq:def:dF:dG}
%DIFDELCMD < \frac{\partial\mat{F}}{\partial{\mat{G}}} = \mathcal{W}(\JJ_k),~~~
%DIFDELCMD < \frac{\partial\mat{F}}{\partial{\mat{G}^H}} = \mathcal{W}(\JJ_{k^*}^T)
%DIFDELCMD < \end{equation}
%DIFDELCMD < 

%DIFDELCMD < The matrix derivative can be viewed as a linear operator on $\COMPLEX^{2\times2}$ that maps an increment
%DIFDELCMD < $\Delta\mat{G}$ 
%DIFDELCMD < to an increment $\Delta\mat{F}$. This is more than just a formal definition: thanks to the isomorphism, the operators 
%DIFDELCMD < given by Eq.~\ref{eq:def:dF:dG}  are Wirtinger derivatives in exactly the same sense that the Jacobians of 
%DIFDELCMD < Eq.~\ref{eq:Jk-fg} are Wirtinger derivatives, the former are simply the $\COMPLEX^{2\times2}$ manifestation of
%DIFDELCMD < the gradient operators in $\COMPLEX^4$, as defined in Sect.~\ref{sec:Wirtinger}. However, operating in 
%DIFDELCMD < $\COMPLEX^{2\times2}$ space allows us to write the larger Jacobians below in terms of simpler matrices composed 
%DIFDELCMD < of operators, with a structure that completely conforms to the scalar derivation.
%DIFDELCMD < 

%DIFDELCMD < It is easy to see that
%DIFDELCMD < \begin{equation}
%DIFDELCMD < \frac{\partial(\mat{GA})}{\partial{\mat{G}}} = \Rop{A},~~~~\frac{\partial(\mat{AG^H})}{\partial{\mat{G^H}}} = \Lop{A},
%DIFDELCMD < \end{equation}
%DIFDELCMD < in the operator sense, since in $\COMPLEX^4$ space the corresponding operation is simply multiplication of 
%DIFDELCMD < a vector by a $\Rop{A}$ or $\Lop{A}$ matrix. Note that this calculus also offers a natural way of taking more
%DIFDELCMD < complicated matrix derivatives. For example,
%DIFDELCMD < \begin{equation}
...
%DIFDELCMD < Let us apply the operator calculus developed above to Eq.~\ref{eq:cal:DI:pol}. We have:
%DIFDELCMD < \begin{equation}
...
by taking the derivative of the Hermitian transpose of the residuals \DIFdelbegin \DIFdel{. We }\DIFdelend \DIFaddbegin \DIFadd{instead. Note that substituting the Hermitian transpose 
for element-by-element conjugation corresponds to a simple reordering of some rows in the conjugate residual vector (i.e.
reordering of the LS equations), which we are always free to do. Let us then }\DIFaddend construct the augmented residual vector of matrices as:
\begin{equation}
\RRr = 
\DIFdelbegin %DIFDELCMD < \Matrix{c}{
%DIFDELCMD <   \RR_{pq} \\ 
...
\bm{\}} \DIFaddend \scriptstyle [pq]=1\dots \Nbl~(p<q) \DIFdelbegin %DIFDELCMD < \\ 
%DIFDELCMD < \} %%%
\DIFdelend \DIFaddbegin \\[1ex] 
\bm{\}} \DIFaddend \scriptstyle [pq]=1\dots \Nbl~(p<q) 
}
...
\DIFdelbegin %DIFDELCMD < \} %%%
\DIFdelend \DIFaddbegin \bm{\big\}} \DIFaddend \scriptstyle [pq]=1\dots 2\Nbl~(p\ne q)\\ 
\end{equation}
and 
\begin{equation}
\RRr = \Matrix{c}{ \mat{R}_{pq}}~~ 
\DIFdelbegin %DIFDELCMD < \} %%%
\DIFdelend \DIFaddbegin \bm{\}} \DIFaddend \scriptstyle [pq]=1\dots 2\Nbl~(p\ne q)\\ 
\end{equation}
This is in complete analogy to the derivations of the unpolarized case. For compactness, let us now define
\newcommand{\YY}{\mat{Y}}
\newcommand{\ZZ}{\mat{Z}}
\begin{equation}
\YY_{pq} = \MM_{pq} \GG^H_q,\DIFaddbegin \DIFadd{~~}\YY\DIFadd{_{qp} = }\MM\DIFadd{^H_{pq} }\GG\DIFadd{^H_p
}\DIFaddend \end{equation}
\DIFdelbegin \DIFdel{and, }\DIFdelend noting that 
\begin{equation}
\YY\DIFaddbegin \DIFadd{^H_{pq} = }\GG\DIFadd{_q }\MM\DIFadd{^H_{pq},~~}\YY\DIFaddend _{qp}^H = \GG_p \MM_{pq}\DIFdelbegin \DIFdel{,
}\DIFdelend \DIFaddbegin \DIFadd{.
}\DIFaddend \end{equation}
\DIFdelbegin \DIFdel{and using }\DIFdelend \DIFaddbegin \DIFadd{Employing }\DIFaddend Eq.~\ref{eq:RH:LH}, \DIFdelbegin \DIFdel{we }\DIFdelend \DIFaddbegin \DIFadd{the $\JJ$ and $\JJ^H$ terms can be written as
}\begin{equation}
\JJ\DIFadd{^H = }\Matrix{c}{
...
\begin{equation}
\label{eq:JHJ:pol}
\JHJ = \DIFdelbegin %DIFDELCMD < \JHJblocks{
%DIFDELCMD <   \mathrm{diag}\sum\limits_{q\ne i} \Rop{\YY_{iq} \YY^H_{iq}} 
%DIFDELCMD < }{
%DIFDELCMD < % & 
%DIFDELCMD < %   \left \{ 
%DIFDELCMD < %   \begin{array}{@{}cc@{}}
%DIFDELCMD < %    \Rop{ \YY^H_{ij}  } \Lop{\YY^H_{ji}},&{\scriptstyle i\ne j} \\
%DIFDELCMD < %    0, &{\scriptstyle i=j}
%DIFDELCMD < %   \end{array} \right . 
%DIFDELCMD < %   \\
%DIFDELCMD <   \left \{ 
%DIFDELCMD <   \begin{array}{@{}cc@{}}
%DIFDELCMD <    \Lop{ Y_{ji}  } \Rop{\YY_{ij}},&{\scriptstyle i\ne j} \\
%DIFDELCMD <    0, &{\scriptstyle i=j}
%DIFDELCMD <   \end{array} \right . 
%DIFDELCMD < }
%DIFDELCMD <   % &
...
\end{equation}%DIFAUXCMD
%DIFDELCMD < \end{equation}
%DIFDELCMD < (Note that this makes use of the property $\Rop{\mat{A}}\Rop{\mat{B}}=\Rop{\mat{BA}}$ and 
%DIFDELCMD < $\Lop{\mat{A}}\Lop{\mat{B}}=\Lop{\mat{AB}}$.) Compare this result to Eq.~\ref{eq:JHJ:DI}.
%DIFDELCMD < 

%DIFDELCMD < For the $\JJ^H\RRr$ term, we can directly apply the linear operators appearing in $\JJ^H$ 
%DIFDELCMD < to the matrices in $\RRr$:
\begin{equation}%DIFAUXCMD
%DIFDELCMD < \begin{equation}
%DIFDELCMD < \JJ^H\RRr = \Matrix{c}{ 
%DIFDELCMD < \sum\limits_{pq} \YY^H_{pq} \RR_{pq} \delta^{i}_p  \\
%DIFDELCMD < \hline\\[-8pt]
%DIFDELCMD < \sum\limits_{pq} \RR_{pq} \YY_{qp} \delta^{i}_q 
%DIFDELCMD < } = \Matrix{c}{
%DIFDELCMD < \sum\limits_{q\ne i} \YY^H_{iq} \RR_{iq} \\
%DIFDELCMD < \hline\\[-8pt]
%DIFDELCMD < \downarrow^H
...
To actually implement GN or LM optimization, we still need to invert the \DIFaddbegin \DIFadd{operator represented by the }\DIFaddend $\JHJ$ matrix \DIFdelbegin \DIFdel{that is represented in 
operator form 
by }\DIFdelend \DIFaddbegin \DIFadd{in 
}\DIFaddend Eq.~\ref{eq:JHJ:pol}. We have two options here. 
\DIFaddbegin 

\DIFaddend The brute-force numerical approach is to substitute the 
$4\times4$ matrix forms of the \DIFdelbegin \DIFdel{$\Rop{~}$ and $\Lop{~}$ }\DIFdelend \DIFaddbegin \DIFadd{$\mathcal{R}$ and $\mathcal{L}$ }\DIFaddend operators (Eqs.~\ref{eq:RA4x4} and \ref{eq:LA4x4}) into the 
equation, thus resulting in conventional matrix,
and then do a straightfoward matrix inversion. 
\DIFaddbegin 

\DIFaddend The second option is to use an analogue of the diagonal approximation described in Sect.~\ref{sec:DI:stefcal}. If we 
neglect the off-diagonal operators of Eq.~\ref{eq:JHJ:pol}, the operator form of the matrix is diagonal, \DIFdelbegin \DIFdel{and the problem 
becomes }\DIFdelend \DIFaddbegin \DIFadd{i.e. the problem 
is again treated as being }\DIFaddend separable per antenna. \DIFdelbegin \DIFdel{The inverse operator 
corresponding to $\Rop{\mat{A}}$ is, trivially, $\Rop{\mat{A}^{-1}}$. We then arrive }\DIFdelend \DIFaddbegin \DIFadd{As for the operators on the diagonal, they are trivially invertable as per 
Eq.~\ref{eq:RAB:LAB}. We can therefore directly invert the operator form
of $\JHJ$ and apply it to $\JJ^H\RRr$, thus arriving at }\DIFaddend a simple per-antenna equation  for the GN update step:
\begin{equation}
...
%DIFDELCMD < \end{equation}
%DIFDELCMD < using the normal shorthand of $\YYd_{pqs} = \MMd_{pqs} \GGdH_q$. The $\JJ^H\RRr$ term is then
%DIFDELCMD < \newcommand{\CCC}{\mathcal{C}}
\DIFadd{\begin{equation}%DIFAUXCMD
%DIFDELCMD < \begin{equation}
%DIFDELCMD < \label{eq:JHR:DD:pol}
%DIFDELCMD < \JJ^H\RRr = \Matrix{c}{
...
For calibration purposes, the sources are clustered in 10 directions using Voronoi tesselation (Fig.~\ref{fig:tessel}). The solution time-interval is set to $4$ minutes, 
...
%DIFDELCMD < \label{app:4x4}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \newcommand{\WW}{\mathbb{W}}
\newcommand{\WWi}{\mathbb{W}^{-1}}
\DIFaddend 

\DIFdelbegin \DIFdel{Here wederive the }\DIFdelend \DIFaddbegin \section{\DIFadd{Operator calculus}}
\label{sec:opcalculus}

\DIFadd{First, let us introduce the vectorization operator ``vec'' and its inverse in the usual (stacked columns) 
way. For a $2\times2$ matrix $\mat{X}$:
}\begin{equation}
\VEC{\mat{X}} \DIFadd{= }\Matrix{c}{x_{11}\\x_{21}\\x_{12}\\x_{22}}\DIFadd{,~~~
}\VECINV{\Matrix{c}{x_{11}\\x_{21}\\x_{12}\\x_{22}}} \DIFadd{= }\mat{X}\DIFadd{,
}\end{equation}
\DIFadd{which sets up an isomorphism between the space of $2\times2$ complex matrices $\COMPLEX^{2\times2}$ and the space 
$\COMPLEX^4$. Note that the ``vec'' operator is linear, in other words the isomorphism preserves linear structure:
}\begin{equation}
\VEC{(\mat{X}+a\mat{Y})}\DIFadd{=}\VEC{\mat{X}}\DIFadd{+a}\,\VEC{\mat{Y}}\DIFadd{,
}\end{equation}
\DIFadd{as well as the Frobenius norm:
}\begin{equation}
\DIFadd{||}\VEC{\mat{X}}\DIFadd{||_F = ||}\mat{X}\DIFadd{||_F.
}\end{equation}

\DIFadd{Consider now the set of all linear operators on $\COMPLEX^{2\times2}$, or $\mathrm{Lin}(\COMPLEX^{2\times2},\COMPLEX^{2\times2}).$
...
\label{eq:Bx}
\mat{B}\bmath{x} \DIFadd{= }\VEC{\mathcal{B}\mat{X}}\DIFadd{,~~~}\mat{X}\DIFadd{=}\VECINV{\bmath{x}}\DIFadd{.
}\end{equation}
\DIFadd{Conversely, any linear operator on 4-vectors $\mat{B}$ can be associated with a linear operator on $2\times2$ matrices by defining
}\begin{equation}
\label{eq:BX}
\DIFadd{\mathcal{B}}\mat{X} \DIFadd{= }\VECINV{\mat{B}\bmath{x}}\DIFadd{,~~~}\bmath{x}\DIFadd{=}\VEC{\mat{X}}\DIFadd{.
}\end{equation}
\DIFadd{Now, the set $\mathrm{Lin}(\COMPLEX^4,\COMPLEX^4)$ is simply the set of all }\DIFaddend $4\times4$ \DIFaddbegin \DIFadd{matrices. Equations~\ref{eq:BX} 
...
\begin{array}{ll}
\WW\DIFadd{\mathcal{B} = }\mat{B}\DIFadd{: }& \mat{B}\bmath{x} \DIFadd{= }\VEC{\,\mathcal{B}\,\VECINV{\bmath{x}}} \\
\WWi\mat{B} \DIFadd{= \mathcal{B}: }& \DIFadd{\mathcal{B}}\mat{X} \DIFadd{= }\VECINV{(\mat{B}\,\VEC{\mat{X}})} 
\end{array}
\end{equation}
\DIFadd{Note that $\mathbb{W}$ also preserves linear structure.
}

\DIFadd{Of particular interest to us are two linear operators on $\COMPLEX^{2\times2}$: 
right-multiply by some $2\times2$ complex matrix $\mat{A}$,  and left-multiply by $\mat{A}$:
}\begin{equation}
\Rop{\mat{A}}\mat{X} \DIFadd{= }\mat{XA}\DIFadd{, ~~~
}\Lop{\mat{A}}\mat{X} \DIFadd{= }\mat{AX} 
\end{equation}
\DIFadd{The $\mathbb{W}$ isomorphism ensures that these operators can be represented as multiplication of 4-vectors 
by specific kinds of $4\times4$ matrices}\DIFaddend .  The matrix outer product proves to be useful here, and in particular the following 
basic relation:
\begin{equation}
\VEC(\mat{A}\mat{B}\mat{C}) = (\mat{C}^T \otimes \mat{A})\,\VEC{\mat{B}}\DIFaddbegin \DIFadd{,
}\DIFaddend \end{equation}
\DIFdelbegin %DIFDELCMD < 
%DIFDELCMD < %%%
\DIFdel{From this }\DIFdelend \DIFaddbegin \DIFadd{from which }\DIFaddend we can derive the matrix operator forms via:
\begin{equation}
\begin{array}{l}
...
\begin{equation}
\label{eq:RA4x4}
\DIFdelbegin %DIFDELCMD < \VEC{\Rop{\mat{A}}[\mat{X}]} %%%
\DIFdelend \DIFaddbegin \WW\Rop{\mat{A}} \DIFaddend = 
\Matrix{cccc}{a_{11}&0&a_{21}&0 \\ 0&a_{11}&0&a_{21} \\ a_{12}&0&a_{22}&0  \\ 0&a_{12}&0&a_{22} }
\DIFdelbegin %DIFDELCMD < \VEC{\mat{X}}
%DIFDELCMD < %%%
\DIFdelend \end{equation}
and
\begin{equation}
\label{eq:LA4x4}
\DIFdelbegin %DIFDELCMD < \VEC{\Lop{\mat{A}}[\mat{X}]} %%%
\DIFdelend \DIFaddbegin \WW\Lop{\mat{A}} \DIFaddend = 
\Matrix{cccc}{a_{11}&a_{12}&0&0 \\ a_{21}&a_{22}&0&0 \\ 0&0&a_{11}&a_{12} \\ 0&0&a_{21}&a_{22} }
\DIFdelbegin %DIFDELCMD < \VEC{\mat{X}}%%%
\DIFdelend \DIFaddbegin \end{equation}

\DIFadd{From this we get the important property that
}\begin{equation}
\label{eq:RH:LH}
[\WW\Rop{\mat{A}}]\DIFadd{^H = }\WW\Rop{\mat{A}^H}\DIFadd{,~~~}[\WW\Lop{\mat{A}}]\DIFadd{^H = }\WW\Lop{\mat{A}^H}\DIFaddend .
\end{equation}
\DIFaddbegin 

\DIFadd{Note that Eq.~\ref{eq:RAB:LAB}, which we earlier derived from the operator definitions, can now
...
%DIF >  Finally, note that the transpose and Hermitian transpose operators in $\COMPLEX^{2\times2}$ space correspond to a swapping
%DIF >  of the second and third rows in $\COMPLEX^4$ space:
%DIF >  \begin{equation}
%DIF >  \label{eq:vec:XH}
%DIF >  \VEC{\mat{X}^T} = \Matrix{c}{x_{11}\\x_{12}\\x_{21}\\x_{22}},~~~
%DIF >  \VEC{\mat{X}^H} = \Matrix{c}{\bar{x}_{11}\\\bar{x}_{12}\\\bar{x}_{21}\\\bar{x}_{22}}.
%DIF >  \end{equation}

\subsection{\DIFadd{Derivative operators and Jacobians}}

\DIFadd{Consider a matrix-valued function of a a matrix argument and its Hermitian transpose,
...
\bmath{f}\DIFadd{(}\bmath{g}\DIFadd{,}\bmath{\bar{g}}\DIFadd{) = }\VEC{\mat{F}(\VECINV{\bmath{g}},(\VECINV{\bmath{\bar{g}}})^T)}\DIFadd{.
}\end{equation}

\DIFadd{Consider now the partial and conjugate partial Jacobians of $\bmath{f}$ with respect to $\bmath{g}$ 
...
\label{eq:Jk-fg}
\JJ\DIFadd{_k = }[ \DIFadd{\partial f_i / \partial g_j }]\DIFadd{,~~~}\JJ\DIFadd{_{k^*} = }[ \DIFadd{\partial f_i / \partial }\bar{g}\DIFadd{_j }]\DIFadd{,
}\end{equation}
\DIFadd{that represent local linear approximations to $\bmath{f}$, i.e. linear operators on $\COMPLEX^4$ that
...
\label{eq:def:dF:dG}
\DIFadd{\frac{\partial\mat{F}}{\partial{\mat{G}}} = }\WWi\DIFadd{(}\JJ\DIFadd{_k),~~~
\frac{\partial\mat{F}}{\partial{\mat{G}^H}} = }\WWi\DIFadd{(}\JJ\DIFadd{_{k^*}^T).
}\end{equation}

\DIFadd{This is more than just a formal definition: thanks to the $\WW$ isomorphism, the operators 
