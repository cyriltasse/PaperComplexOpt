...
\begin{equation}
\label{eq:Jkrk}
\JJ_k=\frac{\partial \vv}{\partial \zz} (\Zz_k), ~\JJ_k^*=\frac{\partial \vv}{\partial \bar{\zz}}(\Zz_k),~
\Rr_k=\Matrix{c}{\rr(\Zz_k)\\\bar{\rr}(\Zz_k)}
\end{equation}
We'll call the $m\times n$ matrices $\JJ_k$ and $\JJ_k^*$ the \emph{partial} and \emph{partial conjugate 
...
\begin{equation}
\label{eq:JJ}
\JJ = \Matrix{cc}{\JJ_k & \JJ^*_k \\ \bar{\JJ^*_k} & \bar{\JJ_k} },
\end{equation}
{\bf with the bottom two blocks being element-by-element conjugated versions of the top two. Note that $\JJ$ is a $2m \times 2n$ matrix.} The GN update step is defined as
\begin{equation}
\label{eq:GN}
...
$pq$ ($1\leq p<q\leq \Na$) 
measures the visibility{\bf\footnote{In principle, the {\em autocorrelation} terms $pp$, corresponding to the
...
\begin{equation}
\label{eq:Jbark}
J_{k^*} = \overbrace{ \Matrix{c}{ g_p m_{pq} \delta^{j}_q } } ^{j=1\dots \Na} \big \}{\scriptstyle [pq]=1\dots \Nbl~(p<q)}
\end{equation}
...
the $\COMPLEX^4$ gradients. This is important becuse it allows us to write the larger Jacobians below 
in terms of matrices composed of operators.

It is easy to see (just from considering the corresponding $4\times4$ matrices, see Appendix~\ref{app:4x4}) that
\begin{equation}
\frac{\partial(\mat{GA})}{\partial{\mat{G}}} = \Rop{A},~~~~\frac{\partial(\mat{AG^H})}{\partial{\mat{G^H}}} = \Lop{A},
\end{equation}
(which makes it explicit that a matrix derivative is an operator on a matrix).
...
We would like to thank the referee, Johan Hamaker for valuable comments that improved the paper.
