...
of the parameter vector, and each operator in the Jacobian becomes a $4\times4$ matrix block), as will be 
shown in Appendix~\ref{sec:opcalculus}.
...
\end{equation}
and think what a meaningful definition for the partial matrix derivative $\partial\mat{F}/\partial\mat{G}_i$ 
would be. A derivative can be thought of as a local linear approximation to $\mat{F}$, i.e.
a linear function mapping an increment in the argument $\Delta\mat{G}_i$ to an increment in the function value:
\begin{equation}
\mat{F}(...,\mat{G}_i+\Delta\mat{G}_i,...) - \mat{F}(...,\mat{G}_i,...) \approx \frac{\partial\mat{F}}{\partial\mat{G}_i}\Delta\mat{G}_i,
\end{equation}
in other words, the partial derivative is a linear matrix operator, in the same sense that the Jacobian is a linear operator. 
...
Appendix~\ref{sec:opcalculus} provides formal definitions of the \emph{Wirtinger matrix derivatives}
\begin{equation}
...
for element-by-element conjugation corresponds to a simple reordering of some rows in the conjugate residual vector, 
which we always are free to do. Let us then construct the augmented residual vector of matrices as:
\begin{equation}
\RRr = 
\Matrix{c}{
  \RR_{pq} \\[1ex] 
...
To actually implement GN or LM optimization, we still need to invert the operator form of the $\JHJ$ matrix in 
Eq.~\ref{eq:JHJ:pol}. We have two options here. The brute-force numerical approach is to substitute the 
...
corresponding to $\Rop{\mat{A}}$ is, trivially, $\Rop{\mat{A}^{-1}}$. We can therefore directly invert the operator form
...
$\mat{X} \in \COMPLEX^{2\times2}$, which we'll write as $\mathcal{B}\mat{X}$, must be equivalent to 
multiplication of $\bmath{x}=\VEC{\mat{X}} \in \COMPLEX^4$ by some $4\times 4$ complex matrix $\mat B$, and vice versa:
\begin{equation}
...
$\mat{F}(\mat{G},\mat{G}^H)$. The ``vec'' operator induces a natural isomorphism between
such functions and 4-vector valued functions of 4-vectors:
\begin{equation}
...
mapping increments in the arguments $\Delta\bmath{g}$ and $\Delta\bmath{\bar{g}}$ to increments 
in the function value $\Delta\bmath{f}$. The isomorphism defined above maps these to the 
equivalent linear operators on $\COMPLEX^{2\times2}$ that represent linear approximations to 
$\bmath{F}$. It is the latter operators that we call \emph{the Wirtinger matrix derivatives}
...
This is more than just a formal definition: thanks to the isomorphism, the operators 
